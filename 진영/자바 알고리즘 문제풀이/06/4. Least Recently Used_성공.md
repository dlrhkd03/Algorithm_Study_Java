...
(맨 앞이 가장 최근이 쓰인 작업이고, 맨 뒤는 가장 오랫도안 쓰이지 않은 작업이다.)
...
public class Main {
    ...
    private static int[] solution(int s, int n, int[] arr) {
        ...
        for (int i = 0; i < n; i += 1) {
            ...
            // 작업이 캐시에 존재한다면 해당 작업이 있는 위치 - 1 기준으로 한 칸씩 오른쪽으로 옮긴다.
            // 작업이 캐시에 존재하지 않는다면 마지막 인덱스 - 1 기준으로 한 칸씩 오른쪽으로 옮긴다.
            // 현재 작업을 캐시 0번 인덱스에 추가한다.
            idx = idx != -1 ? idx : s - 1;
            while (idx > 0) {
