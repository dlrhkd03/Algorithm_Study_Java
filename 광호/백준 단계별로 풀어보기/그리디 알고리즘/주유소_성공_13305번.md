...
| :-------- | :---------- | :---- | :---- | :-------- | :-------- |
| 2 초      | 512 MB      | 32779 | 12447 | 9893      | 37.793%   |
...
public class Main {
    ...
    static void solution() throws IOException {
        ...
        for (int i = 0; i < N; i++) oil_price[i] = Integer.parseInt(st.nextToken());

        int min_price = oil_price[0];
        int idx = 0;
        for (int i = 1; i < N - 1; i++) {
            if (oil_price[i] < min_price) {
                for (int j = idx; j < i; j++) {
                    result += (long) min_price * dis[j];
                }
                idx = i;
